...
module.exports = function(options) {
  ...
  function execute (postUrl, req, res) {
    ...
    if (!options.jwt) {
      ...
      var escapedWctx = utils.escape(utils.escape(options.wctx || req.query.wctx)); // we need an escaped value for RequestSecurityTokenResponse.Context
      var escapedAssertion = utils.escape(signedAssertion); // we need an escaped value for RequestSecurityTokenResponse.Context
      signedAssertion = '<t:RequestSecurityTokenResponse Context="'+ escapedWctx + '" xmlns:t="http://schemas.xmlsoap.org/ws/2005/02/trust"><t:RequestedSecurityToken>' + escapedAssertion + '</t:RequestedSecurityToken></t:RequestSecurityTokenResponse>';
    } else {
      signedAssertion = jwt.sign(user, options.key.toString(), {
