  ...
  function execute (postUrl, req, res, next) {
    ...
    if (!options.jwt) {
      ...
      saml11.create({
        ...
        var escapedWctx = utils.escapeWctx(utils.escapeWctx(ctx)); // we need an escaped value for RequestSecurityTokenResponse.Context
        var escapedAssertion = utils.escapeAssertion(assertion); // we need an escaped value for RequestSecurityTokenResponse.RequestedSecurityToken
